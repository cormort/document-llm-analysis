...
class RAGQueryRequest(BaseModel):
    ...
    use_query_expansion: bool = Field(
        default=False, description="Enable LLM-based query expansion"
    )
    use_compression: bool = Field(
        default=False, description="Enable context compression"
    )
    compression_method: str = Field(
        default="extractive", description="Compression method: extractive or summary"
    )
    use_web_search: bool = Field(
        default=False, description="Enable web search verification"
    )
    use_strategy: bool = Field(
        default=False, description="Apply business strategy framework"
    )
    config: LLMConfig = Field(default_factory=LLMConfig)
    fast_config: LLMConfig | None = Field(
        default=None, description="Optional config for fast/background tasks"
    )
...
class RAGIndexResponse(BaseModel):
    ...
    collection_name: str | None = Field(
        default=None, description="Created collection name"
    )
...
class AstuteRAGQueryRequest(BaseModel):
    ...
    question: str = Field(..., description="User question")
    collection_names: list[str] = Field(
        default_factory=list, description="Document collections to search (optional)"
    )
    n_results: int = Field(default=5, description="Number of results to retrieve")
    use_hybrid: bool = Field(default=True, description="Enable hybrid search")
    use_rerank: bool = Field(default=True, description="Enable reranking")
    force_retrieval: bool = Field(
        default=False, description="Always perform retrieval even if LLM is confident"
    )
    config: LLMConfig = Field(default_factory=LLMConfig)
...
class InternalKnowledgeResponse(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0, le=1, description="Self-assessed confidence (0-1)"
    )
    key_facts: list[str] = Field(
        default_factory=list, description="Extracted key facts"
    )
...
class ConflictInfo(BaseModel):
    ...
    conflict_type: str = Field(
        ..., description="Type of conflict: factual, semantic, scope"
    )
...
class ReliabilityDistribution(BaseModel):
    ...
    medium: int = Field(
        default=0, description="Count of medium reliability facts (0.4-0.7)"
    )
...
class AstuteRAGQueryResponse(BaseModel):
    ...
    answer: str = Field(..., description="Generated answer")
    confidence: float = Field(
        ..., ge=0, le=1, description="Overall confidence in the answer"
    )
    sources_used: list[SourceInfo] = Field(
        default_factory=list, description="Sources used in the answer"
    )
    conflicts_noted: list[ConflictInfo] = Field(
        default_factory=list, description="Knowledge conflicts detected"
    )
    knowledge_type: str = Field(
        ..., description="Primary knowledge source: internal, external, hybrid"
    )
    ...
    consolidation_summary: str = Field(
        default="", description="Summary of knowledge consolidation"
    )
...
class VerifyAnswerRequest(BaseModel):
    ...
    collection_names: list[str] = Field(
        ..., description="Document collections to verify against"
    )
    config: LLMConfig = Field(default_factory=LLMConfig)
...
class VerifyAnswerResponse(BaseModel):
    ...
    supporting_facts: list[str] = Field(
        default_factory=list, description="Facts supporting the answer"
    )
    contradicting_facts: list[str] = Field(
        default_factory=list, description="Facts contradicting the answer"
    )
