...
class ReportGenerateRequest(BaseModel):
    ...
    selected_skills: list[str] = Field(
        default_factory=list, description="IDs of selected agent skills"
    )
    file_path: str | None = Field(
        default=None, description="Source file path for the report"
    )
    user_instruction: str = Field(
        default="", description="Optional customization instructions"
    )
